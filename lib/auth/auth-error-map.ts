const AUTH_ERROR_MAP: Record<string, string> = {
  ...
  "Phone logins are disabled": "auth_phone_provider_disabled",
  "Phone authentication is disabled": "auth_phone_provider_disabled",
  "Phone auth provider must be enabled": "auth_phone_provider_disabled",
...
export function getAuthErrorKey(error: { message?: string; code?: string }): string {
  ...
  if (error.message) {
    ...
    if (
      lowerMsg.includes("phone logins are disabled") ||
      lowerMsg.includes("phone provider disabled") ||
      lowerMsg.includes("phone auth provider must be enabled")
    ) {
      return "auth_phone_provider_disabled";
    }
    if (lowerMsg.includes("phone sign-up") || lowerMsg.includes("phone signups")) {
      return "auth_phone_exists";
    }
    ...
    if (lowerMsg.includes("weak password") || lowerMsg.includes("at least 8")) return "auth_weak_password";
    if (lowerMsg.includes("email link") && (lowerMsg.includes("invalid") || lowerMsg.includes("expired"))) {
      return "auth_link_expired";
    }
    if (lowerMsg.includes("user not found") || lowerMsg.includes("email not found")) return "auth_generic_error";
    if (lowerMsg.includes("network") || lowerMsg.includes("fetch")) return "auth_network_error";
    if (lowerMsg.includes("session") || lowerMsg.includes("not authenticated")) return "auth_session_expired";
  }
