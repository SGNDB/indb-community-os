    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">{labels.moderationDescription}</p>
        </div>
      </div>
      ...
      <div className="grid gap-4">
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="font-bold text-foreground">{labels[report.reasonKey]}</h3>
                  <StatusBadge status={report.status} label={labels.statusPending} />
                ...
                <div className="text-sm text-muted-foreground mt-1 flex flex-wrap items-center gap-x-4 gap-y-1">
                  <span>{labels.conversation}: <span className="font-medium text-foreground">{report.conversationId}</span></span>
                  <span className="flex items-center gap-1">
                    {labels.reportedBy}: <AdminAvatar profile={report.reporter} className="w-5 h-5 ml-1" /> <span className="font-medium text-foreground">{report.reporter.username}</span>
                  </span>
                  <span>{labels[report.reportDateKey]}</span>
                </div>
              </div>
            </div>
        ...
        {mockReports.length === 0 && (
          <div className="p-8 text-center bg-muted/30 rounded-2xl border border-dashed border-border">
            ...
            <p className="text-sm text-muted-foreground">{labels.allReportedReviewed}</p>
          </div>
        )}
      </div>
    </div>
