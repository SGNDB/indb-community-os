...
import { getUserConversations, getConversationById } from "@/lib/data/conversations";
...
export default async function MessagesPage({
  ...
  if (sp.conversation) {
    const conversationId = sp.conversation;
    const inboxConversation = conversations.find((item) => item.id === conversationId) ?? null;
    const selectedConversation = await getConversationById(conversationId, user.id, inboxConversation);
  ...
  return (
    <section className="flex h-[calc(100dvh-3.5rem-var(--safe-top))] min-h-0 w-full overflow-hidden bg-background md:h-full">
      <div className="flex min-h-0 w-full flex-col md:w-[30%] md:min-w-[17rem] md:shrink-0 md:border-e md:border-border/70">
