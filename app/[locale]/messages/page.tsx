...
export default async function MessagesPage({
  ...
  return (
    <section className="mx-auto flex h-[calc(100dvh-8.75rem)] min-h-[560px] w-full overflow-hidden rounded-lg border border-border/70 bg-background shadow-sm md:h-[calc(100dvh-7.5rem)]">
      <div className={`${selectedConversation ? "hidden md:flex" : "flex"} w-full flex-col md:w-[31%] md:min-w-[18rem] md:shrink-0 md:border-e md:border-border/70`}>
        <ConversationList initialConversations={conversations} currentUserId={currentUserId} />
      </div>

      {selectedConversation ? (
        <div className="flex min-w-0 flex-1 flex-col md:w-[69%]">
          <ConversationChat
        ...
        <div className="hidden min-w-0 flex-1 items-center justify-center md:flex md:w-[69%]">
