-- ============================================================
-- MEMORY REACTIONS
-- Four reaction types: love, respect, nostalgia, important
-- One active reaction per user per memory (unique constraint)
-- ============================================================
create table if not exists public.memory_reactions (
  id uuid primary key default gen_random_uuid(),
  memory_id uuid not null references public.memories(id) on delete cascade,
  user_id uuid not null references public.profiles(id) on delete cascade,
  reaction_type text not null check (reaction_type in ('love', 'respect', 'nostalgia', 'important')),
